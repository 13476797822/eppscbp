(function() {
	...
	function bindDataIntoElements(a) {
		...
		$(l.resultNumBox).innerHTML = "<b>编号</b><b class='code'>币种</b>";
		
		if(e != 0 && e != 1) {
			$(l.resultNumBox).innerHTML += "<i>名称</i>"
		}
		if(o['isPage']) {
		...
		for(var i = a; i < b; i++) {
			if(u[i]) {
				var d = u[i][p['subval']],
					...
					f = u[i][p['py']],
					k = u[i][p['bz']],
					g = '<span class=' + m.title + '>' + e + '</span>'+'<span class=' + m.title + '>' + k + '</span>' +'<span class="name" title="'+f+'">'+ f + '</span>' +'';
				var h = e;
	...
	function getElemArray() {
		var a = [],
			b = document.getElementsByClassName(m.exprElem);			
		for(var i = 0; i < b.length; i++) {
			a.push(b[i])
		}
		return a
