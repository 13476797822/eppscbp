        ...
        Select.prototype = {
            ...
            initClickSelect: function() {
                ...
                this.$jrSelect.find('.select-input').click(function(e) {
                    ...
                    if (!self.isSelectBoxVisible($this) && !$this.parents('.jr-select').hasClass('disabled')) {
            ...
            arrowOpen: function($selectInput) {
                $selectInput.siblings('.select-input-arrow-icon')
                    .removeClass('arrow-icon-closed')
                    .addClass('arrow-icon-open');
            },

            arrowClosed: function() {
                this.$jrSelect.find('.select-input-arrow-icon')
            ...
            initSelectValue: function() {
                this.$jrSelect.find('.sel-val').click(function() {
                    var $this = $(this),
                        ...
                        $input1 = $parent.find(".select-input-hidden"),
                        $input2 = $parent.find('.select-input'),
                        $selectBox = $parent.find('.select-box'),
                        val = $this.attr('value'),
                        key = $this.attr('key'),
                        text = $this.text();
                    $input1.val(val);
                    $input2.val(text).attr('key', key).trigger('change');
                    $selectBox.hide();
                });
            },
