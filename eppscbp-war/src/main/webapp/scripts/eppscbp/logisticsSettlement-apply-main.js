...
require(['jquery','template','select','searchableSelect','validate','dialog','ajaxFileUpload'], function($,template,select,searchableSelect,validate,dialog,ajaxFileUpload) {
   ...
   $('#select-currency').on("click",".sel-val",function(){
		...
		$.ajax({
      	...
      	url:sysconfig.ctx+"/logisticsSettlement/fileImport/arrivalQuery.htm",
      	dataType: "JSON",
      	async:false,
        	...
        	//清空商户列表
      		...
      		if(data.success == true){		 	           		 	
      			...
      			for(var i = 0;i < arrivalInfoList.length;i++){
      				...
      				var option=$('<li class="select-item"><a class="sel-val" href="javascript:void(0);" key="1" value="'+payerBankCard+'" code="'+ payeeMerchantCode+'" data="'+ arrivalNoticeList +'">'+payerBankCard+'</a></li>');
    ...
    $('#select-merchant').on("click",".sel-val",function(){
		...
		$.ajax({
	      	type: "POST",
	      	data:{"payeeMerchantCode":$(this).attr("code")},
	      	url:sysconfig.ctx+"/logisticsSettlement/fileImport/isOpenColl.htm",
	      	dataType: "JSON",
	      	async:false,
	      	success: function(data) {
	      		if(data.success == true){		 	           		 	
	      			var collectionType = data.result.collectionType;
	      			if(typeof(collectionType) == "undefined"||"1" != collectionType){
	      				$("#select-box-merchant").parent().find('.tips').append('境外商户未开通收结汇功能').addClass('err');   
	      				return;
	      			}	      			
	      		}else{			
					$("#select-box-merchant").parent().find('.tips').append(data.responseMsg).addClass('err'); 
				}
	     	}
	     	});
		$('#arrival').searchableSelect();
    ...
    $(document).on("click","#J_placeOrder",function(){
		...
		formObj.attr("action", sysconfig.ctx+"/logisticsSettlement/fileImport/applySubmit.htm");
		formObj.submit(); 
    ...
    var sjhApply ={
            ...
            uploadFile:function () {
                ...
                $file.unbind().live('change', function() {
                    ...
                    $.ajaxFileUpload({
                        url: sysconfig.ctx+"/logisticsSettlement/fileImport/fileSubmit.htm",
                        type: 'POST',
                        fileElementId: 'fileUpload',
                        data: {currency:currency, receiveAmt:receiveAmt,collAndSettleFlag:"1"},                       
                        dataType: 'text',
                        success: function(data) {
                        	//若页面过期跳转至首页
                        	isIntercepted(data);
