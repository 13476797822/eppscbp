<meta charset="UTF-8">
            ...
            <form id="fillCross" id="single-pay"  method="post" name="f0">
                <ul class="input-list" id="accountContent clearfix">
                    ...
                    <li class="input-item clearfix">
                        ...
                        <input type="text" class="input remarks show" name="detailAmount" id="detailAmount" min="1" max="20000" <#if criteria.detailAmount != 0> value="${criteria.detailAmount}" </#if> placeholder="小于等于20000笔">
                        <div class="clear"></div>
                    </li>
                    <input type="text" name="requestUrl" value="/goodsTrade/goodsPay/fileSubmit.htm" class="input file-path" style="position:absolute;z-index:-1;font-size:0;border:0;left:-1000%;">
    ...
    <script type="text/javascript">
		...
		function submitOrder(){
    	    ...
    	    formObj.attr("action", sysconfig.ctx+"/goodsTrade/goodsPay/applySubmit.htm");
    	    formObj.submit(); 
    	...
    	function queryMerchantMeg(){
	    	...
	    	$.ajax({
             	...
             	success: function(data) {
             		...
             		$("#receiveName").parent().find("p.tip-red").remove();
            	 	if(data.success == "s"){
	    ...
	    $(document).on("click",".balance-btn",function(){
	        ...
	        $.ajax({
	            ...
	            url:sysconfig.ctx+"/goodsTrade/goodsPay/queryBalance.htm",
	            dataType: "json",
	            success: function(data) {
	            	var html,title;
	            	if(data.success == "s"){
